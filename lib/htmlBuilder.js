module.exports = function(file, blocks, options, push, callback) {
  ...
  if (options && options.pathSuffix) {
    if (typeof options.pathSuffix == 'function') {
      pathSuffix += options.pathSuffix(file);
    } else {
      ...
      if (typeof block == 'string') {
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          ...
          if (path.extname(file.path) == '.js')
            html[i] = '<script src="' + name.replace(path.basename(name), path.basename(file.path)) + pathSuffix + '"></script>';
          resolve();
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          push(file);
          html[i] = '<link rel="stylesheet" href="' + name.replace(path.basename(name), path.basename(file.path)) + pathSuffix + '"'
