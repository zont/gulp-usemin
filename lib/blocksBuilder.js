  ...
  function getFiles(content, reg, alternatePath) {
      ...
      content.replace(cssMediaReg, function(a, media) {
        ...
        if (!cssMediaQuery) {
          ...
          if (cssMediaQuery != media)
            throw new gutil.PluginError('gulp-usenew', 'incompatible css media query for ' + a + ' detected.');
        }
      });
    }
    ...
    for (var i = 0, l = paths.length; i < l; ++i) {
      ...
      if(filepaths[0] === undefined && !options.skipMissingResources) {
        throw new gutil.PluginError('gulp-usenew', 'Path ' + paths[i] + ' not found!\nThe file is: ' + file.history);
      } else {
